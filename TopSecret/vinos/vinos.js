...
function buyProduct(k) {
    ...
    if (cantidad > 0 && vino[k].stock >= cantidad) {
        vino[k].stock -= cantidad; // Restar del stock
        ...
        showCarrito();
        showProducts(); // Actualizar la visualización de los productos
    } else {
...
function loadProducts() {
    vino = [
        { nombre: "Habla de la tierra", precio: 20.99, stock: 10, imagen: "assets/img/vinos/vino1.jpg" },
        { nombre: "Finca de la Estacada", precio: 15.00, stock: 13, imagen: "assets/img/vinos/vino2.jpg" },
        { nombre: "Peñascal", precio: 10.00, stock: 30, imagen: "assets/img/vinos/vino3.jpg" },
        { nombre: "Cabernet Franc", precio: 18.50, stock: 3, imagen: "assets/img/vinos/vino4.jpg" },
        { nombre: "Juan Gil", precio: 5.00, stock: 10, imagen: "assets/img/vinos/vino5.jpg" }
    ];
}
...
function showProducts() {
    ...
    for (let k in vino) {
        let disableBuyButton = vino[k].stock === 0 ? 'disabled' : '';
        let stockWarning = vino[k].stock === 0 ? '<span class="text-danger">Sin stock</span>' : '';
        ...
        listProductos += `<div class="col-md-6 mb-4">
            <div class="card border-dark">
                ...
                <div class="card-body">
                    ...
                    <p class="card-text">${stockWarning}</p>
                    <input type="number" id="txt${k}" class="form-control mb-2" value="1" min="1" max="${vino[k].stock}">
                    <button type="button" class="btn btn-primary" onclick="buyProduct(${k})" ${disableBuyButton}>Comprar</button>
                </div>
            </div>
        </div>`;
    }
